...
def test_symmetric_slab_mode_count_matches_v_number(symmetric_slab, thickness_nm, polarization):
    problem = symmetric_slab(thickness_nm, polarization=polarization, method="FEM")
...
def test_modes_are_sorted_by_descending_neff(symmetric_slab):
    problem = symmetric_slab(1100.0, method="FEM")
    results = SOLVER.solve(problem)
...
def test_neff_within_guided_bounds(soi_problem):
    problem = soi_problem(method="FEM")
    results = SOLVER.solve(problem)
...
def test_field_profile_is_normalized_and_decays_at_box_edges(soi_problem):
    problem = soi_problem(method="FEM")
    results = SOLVER.solve(problem)
    for result in results:
        assert result.field is not None
        assert len(result.field.x_nm) == len(result.field.amplitude)
        assert abs(result.field.amplitude).max() == pytest.approx(1.0)
        # Dirichlet nas bordas da caixa: campo deve estar ~zero nas extremidades da malha
        assert abs(result.field.amplitude[0]) == pytest.approx(0.0, abs=1e-9)
        assert abs(result.field.amplitude[-1]) == pytest.approx(0.0, abs=1e-9)


def test_no_guided_mode_returns_empty_list(no_mode_problem):
    problem = no_mode_problem(method="FEM")
